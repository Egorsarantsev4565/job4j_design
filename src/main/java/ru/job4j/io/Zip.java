...
public class Zip {
    ...
    public static void packFiles(List<Path> sources, Path target) {
        try (ZipOutputStream zip = new ZipOutputStream(
            ...
            for (Path paths : sources) {
                zip.putNextEntry(new ZipEntry(paths.toFile().getPath()));
                try (
                        BufferedInputStream out = new BufferedInputStream(
                                new FileInputStream(paths.toFile().getPath()))) {
    ...
    public static void main(String[] args) {
        ...
        try {
            List<Path> search = Search.search(
                    directory, s -> !s.toFile().getName().endsWith(exclude));
